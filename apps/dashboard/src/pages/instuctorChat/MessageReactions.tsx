...
export default function MessageReactions({
  ...
  // internal state used when open isn't controlled
  ...
  // countsMap: prefer reactionCounts prop, otherwise compute from reactions (dedupe by user)
  ...
  // when user selects a reaction
  const handleSelect = useCallback(
    (type: ReactionType, e?: React.MouseEvent) => {
      e?.stopPropagation();
    ...
    [messageId, onReact, onRemove, userReactionFromReactions, setOpen],
  );
  ...
  const handleRemoveClick = useCallback(
    ...
    [messageId, onRemove, setOpen],
  );

  const popoverAnchoredClass = isMe ? 'right-0' : 'left-0';

  return (
    // add `group` so parent hover (if any) can trigger the small button via group-hover
    <div ref={containerRef} className="flex items-center select-none relative z-20 group">
      ...
      {summaryTwo.length > 0 && (
        <div
          // allow summary clicks to open the popover too (optional UX)
          onClick={(e) => {
            e.stopPropagation();
            toggleOpen();
          }}
          className="inline-flex items-center gap-1 rounded-md px-1 py-0.5 text-xs shadow-sm bg-white cursor-pointer"
          aria-hidden
        >
          {summaryTwo.map((t, i) => (
      ...
      {/* Trigger: visible on group hover/focus or when open */}
      <button
        onClick={(e) => {
          e.stopPropagation();
          toggleOpen();
        ...
        type="button"
        className={
          'ml-2 rounded-full p-1 hidden md:inline-flex  hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-offset-1 focus:ring-primary ' +
          (open
      ...
      {/* Popover: pointer-events enabled only when open */}
      <div
        aria-hidden={!open}
        role="menu"
      ...
      >
        ...
        >
          <div className="rounded-full bg-white p-2 shadow-lg flex gap-2 items-center transform transition-all duration-180 ease-out">
            {(['like', 'love', 'wow', 'laugh', 'sad', 'angry'] as ReactionType[]).map((type, i) => {
              ...
              return (
                ...
                >
                  <span className="pointer-events-none select-none">{REACTION_EMOJI[type]}</span>
                </button>
              );
            })}
          ...
          {/* small "more" menu for mobile / condensed UI */}
