// src/controllers/mediaHooks.ts
import { Request, Response } from 'express';
import mongoose from 'mongoose';
import { asyncHandler } from '@elearning/lib';
import { Course, Chapter } from '@elearning/models';

/**
 * POST /api/media/hooks/status
 * Protected with ADMIN_API_KEY (bearer token).
 */
export const postMediaStatus = asyncHandler(async (req: Request, res: Response) => {
  // Basic auth check
  ...
  } else if (videoId && mongoose.isValidObjectId(String(videoId))) {
  ...
  const makeVideoStatusSet = () => {
    const set: any = {};
    if (step !== undefined) set['videoStatus.step'] = step;
    if (typeof pct === 'number') set['videoStatus.pct'] = pct;
    if (s3Prefix !== undefined) set['videoStatus.s3Prefix'] = s3Prefix;
    if (playbackUrl !== undefined) set['videoStatus.playbackUrl'] = playbackUrl;
    if (error !== undefined) set['videoStatus.error'] = error;
    if (videoJobId !== undefined) set['videoStatus.videoJobId'] = videoJobId;
    if (videoId !== undefined) set['videoStatus.videoId'] = videoId;
  ...
  Object.keys(videoStatusSet).forEach((k) => {
    if (videoStatusSet[k] === undefined) delete videoStatusSet[k];
  });
  ...
  if (incomingLectureId && courseId && mongoose.isValidObjectId(courseId)) {
    try {
      ...
      const query = { courseId:  new mongoose.Types.ObjectId(courseId), 'lectures._id': new mongoose.Types.ObjectId(lectureObjectId) };
      const updated = await Chapter.findOneAndUpdate(query, { $set: setObj }, { new: true }).lean();

      if (!updated) {
        // Not found: maybe the lecture is saved as string IDs; fall back to searching without objectId conversion
        const fallbackQuery: any = { courseId: courseId, 'lectures._id': lectureObjectId };
        const updatedFallback = await Chapter.findOneAndUpdate(fallbackQuery, { $set: setObj }, { new: true }).lean();
        if (!updatedFallback) {
  ...
  if (courseId && mongoose.isValidObjectId(courseId)) {
    try {
      const course = await Course.findByIdAndUpdate(courseId, { $set: videoStatusSet }, { new: true }).lean();
      if (!course) {
